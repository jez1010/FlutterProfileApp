...
class ProfileRepository {
  ...
  Future<List<dynamic>> getProfileDetails({String? personId}) async{
    ...
    if (personId == null && _cachedProfile != null && targetId == userId) return _cachedProfile!;
  ...
  Future<void> pushToDatabase(List<dynamic> inputs, BuildContext context) async {
    // 0 = username, 
    //1 = first name, 
    //2 = last name, 
    //3 = tags, 
...
Future<Map<String, List<dynamic>>> searchProfiles({String? user_id, String? term}) async {
  try {
    ...
    final List<List<dynamic>> structuredResults = response.map((profile) {
      ...
      String names = (name1.isNotEmpty || name2.isNotEmpty) ? "$name1|$name2" : "$name1|$name2";
